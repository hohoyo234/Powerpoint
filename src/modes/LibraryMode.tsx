...
export default function LibraryMode({ modeToggle }: { modeToggle: React.ReactNode }) {
  ...
  const filtered = useMemo(() => {
    ...
    return songs.filter((s) => {
      ...
      if (!nq) return true;
      return [s.title, s.englishTitle, s.producer, s.lyrics].some((f) => (f || '').toLowerCase().includes(nq));
    });
  }, [songs, q, filter]);
  ...
  const saveEdit = () => {
    ...
    updateById(editing.id, { title: editing.title, englishTitle: editing.englishTitle, producer: editing.producer, lyrics: editing.lyrics, englishLyrics: editing.englishLyrics });
              ...
              <div key={s.id} className="flex items-center gap-3 px-4 sm:px-6 py-3.5 hover:bg-[#F9F7F5] transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    {s.producer && <span className="text-[10px] text-outline/40 font-bold truncate">{s.producer}</span>}
                    <span className={`text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded ${hasLyrics ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{hasLyrics ? '有歌词' : '仅歌名'}</span>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[32px] shadow-2xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <L label="歌名"><input value={editing.title} onChange={(e) => setEditing({ ...editing, title: e.target.value })} className="li" /></L>
              <L label="英文名 / 副标题"><input value={editing.englishTitle || ''} onChange={(e) => setEditing({ ...editing, englishTitle: e.target.value })} className="li" /></L>
            </div>
            <L label="制作人 / 来源"><input value={editing.producer || ''} onChange={(e) => setEditing({ ...editing, producer: e.target.value })} className="li" /></L>
            <div className="mt-3"><L label="歌词(每行一句,空行换页;用 [副歌] 标记重复段)"><textarea value={editing.lyrics} onChange={(e) => setEditing({ ...editing, lyrics: e.target.value })} rows={8} className="li resize-none leading-relaxed" /></L></div>
            <div className="mt-3"><L label="翻译 / 对照歌词(可选)"><textarea value={editing.englishLyrics || ''} onChange={(e) => setEditing({ ...editing, englishLyrics: e.target.value })} rows={4} className="li resize-none leading-relaxed" /></L></div>
